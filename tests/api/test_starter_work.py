...
@pytest.fixture
def match_client() -> Generator[TestClient, Any, None]:
...
def test_match_ranks_full_overlap_above_no_overlap(match_client: TestClient) -> None:
    response = match_client.post(
        f"{_BASE}/match",
        json={
            "hire_competencies": [{"key": "kotlin", "label": "Kotlin"}],
            "pool": [
                {"source_id": "no-match", "title": "No match", "competency_keys": []},
                {
                    "source_id": "match",
                    "title": "Match",
                    "competency_keys": ["kotlin"],
                },
            ],
        },
    )
    ...
    assert [r["task"]["source_id"] for r in ranked] == ["match", "no-match"]
    assert ranked[0]["matched_competency_keys"] == ["kotlin"]


def test_match_empty_pool_returns_empty_list(match_client: TestClient) -> None:
    response = match_client.post(
        f"{_BASE}/match",
        json={"hire_competencies": [{"key": "kotlin", "label": "Kotlin"}], "pool": []},
    )
